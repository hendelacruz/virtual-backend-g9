...
class ProductoController(APIView):
    ...
    def put(self, request:Request, id):
        ...
        # 2. modificare los productos proveidos
        serializador = ProductoSerializer(data=request.data)#data = lo que me envia en el body
        if serializador.is_valid():
            # instance es lo que queremos actualizar
            serializador.update(instance=productoEncontrado, validated_data=serializador.validated_data)

            # 3. guardare y devolvere el producto actualizado
            return Response(data={
    ...
    def post(self, request: Request):
        ...
        if data.is_valid():
          ...
          # print(data.data)
            ...
            # si la direccion no es vacia(tiene contenido) su valor seguira siendo direccion, caso contrario
            # extraeremos la direccion del resultado de APIPERU
            #direccion = direccion if direccion else data.get('direccion_completa') # seria asi si la direccion es distinta a la que figura en sunat
...
class BuscadorClienteController(RetrieveAPIView):
    ...
    def get(self, request: Request):
        ...
        # agregar los test para el cliente controler y busqueda
        # dar la opcion que se puedan enviar el documento y nombre a la vez y que se haga el filtro de ambos si es que se provee
          return Response(data={
            'content': data.data
          })
