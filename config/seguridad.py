...
class Usuario:
  ...
  def __str__(self):
      ...
      # el % idica que lo pasa a string( ya sea el id y el username)
      # return "usuario con el id='{}' y username = '{}'".format(self.id, self.username)

def autenticador(username, password):
  '''Funcion encargada en mi JWT de validar las credenciales, valida si son ingresadas correctamente y
  luego valida si es el usuario'''
  if username and password:
    #buscamos al usuario en la base de datos segun el correo
